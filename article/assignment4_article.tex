...
\\
The values shown in Table \ref{tab:table1} are representing the entirety of the lattice. To transition from lattice wide values to single object values, one simply have to divide by the size of the lattice, $4$ in the above case, and multiply in the reverse case.
...
\\
If the initial spins are all upwards like in \figref{acceptanceupspin}, the acceptance becomes more linear. The line for $T = 1.0$ is more squiggly than the straighter line for $T = 2.4$. The ladder does not spike for few Monte Carlo cycles in this case.
\\
...
\\
The run time for higher order lattices increased as the lattice size increased. The run times for some lattice sizes are shown in the table below:
